...
Citizen.CreateThread(function()
  while true do
    ...
    if takePhoto ~= true then
      if IsControlJustPressed(3, KeyOpenClose) and GetLastInputMethod(2) then
      ...
      if menuIsOpen == true then
        for _, value in ipairs(KeyToucheCloseEvent) do
          if IsControlJustPressed(3, value.code) then
...
RegisterNUICallback('callEvent', function(data, cb)
  ...
  elseif data.eventName == "policestdr" then
    local eventName = "police"
    print(tostring(myPhoneNumber))
    ShowNotificationMenuCivil2("~h~Vous allez etre mis en relation avec un ~b~officier de police")
    TriggerServerEvent("call:makeCallphone", eventName, myPhoneNumber, GetPlayerServerId(player))
  elseif data.eventName == "cardealerstdr" then
    local eventName = "cardealer"
    print(tostring(myPhoneNumber))
    ShowNotificationMenuCivil2("~h~Vous allez etre mis en relation avec un ~b~concessionaire")
    TriggerServerEvent("call:makeCallphone", eventName, myPhoneNumber, GetPlayerServerId(player))
  elseif data.eventName == "ambulancestdr" then
    local eventName = "ambulance"
    print(tostring(myPhoneNumber))
    ShowNotificationMenuCivil2("~h~Vous allez etre mis en relation avec un ~b~médecin")
    TriggerServerEvent("call:makeCallphone", eventName, myPhoneNumber, GetPlayerServerId(player))
  else
    if data.data ~= nil then 
...
RegisterNUICallback('takePhoto', function(data, cb)
  ...
  if hasFocus == true then
    ...
    Citizen.Wait(0)
		...
		if IsControlJustPressed(3, 27) then -- Toogle Mode
