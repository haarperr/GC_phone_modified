    ...
    while true do
        ...
        if IsControlJustPressed(1, 246) and callActive then
            if isCall == false then
                if callphone == true then
                  TriggerServerEvent("call:getCall", work)
                  SendNotification("~b~Vous avez pris l'appel téléphonique !")
...
AddEventHandler("call:callIncoming2", function(job, myPhoneNumber)
    callphone = true
    callActive = true
    work = job
    ...
    SendNotification("Appuyez sur ~b~Y~s~ pour prendre l'appel ou ~r~N~s~ pour le refuser")
    if work == "police" then
        SendNotification("~b~APPEL EN COURS:~w~ " ..tostring(myPhoneNumber))
        --SendNotification("Appuyez sur ~g~Y~s~ pour prendre l'appel ou ~g~L~s~ pour le refuser")
    elseif work == "cardealer" then
        SendNotification("~o~APPEL téléphonique EN COURS:~w~ " ..tostring(myPhoneNumber))
    elseif work == "taxi" then
        SendNotification("~y~APPEL EN COURS:~w~ " ..tostring(msg))
        --SendNotification("Appuyez sur ~g~Y~s~ pour prendre l'appel ou ~g~L~s~ pour le refuser")
    elseif work == "ambulance" then
